...
def list_images(directory):
    ...
    return sorted([p.name for p in directory.iterdir() if p.is_file() and p.suffix.lower() in valid])
...
def load_models():
    ...
    if not SAR_MODEL.exists() or not OPTICAL_MODEL.exists():
        raise FileNotFoundError("Model checkpoints not found. Run train.py first.")
    ...
    with torch.no_grad():
        for start in range(0, len(selected_files), BATCH_SIZE):
            ...
            for filename in batch_files:
                image = Image.open(optical_dir / filename).convert("RGB")
                batch_images.append(TRANSFORM_OPTICAL(image))
...
def encode_query_image(sar_encoder, sar_dir, query_file=None, uploaded_file=None):
    ...
    tensor = TRANSFORM_SAR(image).unsqueeze(0).to(DEVICE)
    with torch.no_grad():
...
def retrieve_matches(query_file=None, uploaded_file=None, k=6, limit=2000, refresh_cache=False):
    ...
    if sar_dir is None or optical_dir is None:
        raise FileNotFoundError("Dataset not found. Add data/SAR + data/Optical or data/Paired_SAR_Optical_images.")
    ...
    query_name, query_embedding = encode_query_image(sar_encoder, sar_dir, query_file, uploaded_file)
    ...
    top_scores, top_indices = torch.topk(similarities, min(k, len(indexed_files)))
